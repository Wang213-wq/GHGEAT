...
df_train = pd.read_csv('D:\\化工预测\\论文复现结果\\GH-GAT\\scr\\temperature_dependency\\predictions\\train_predictions.csv')
df_test = pd.read_csv('D:\\化工预测\\论文复现结果\\GH-GAT\\scr\\temperature_dependency\\predictions\\test_predictions.csv')
...
folder = 'D:\\化工预测\\论文复现结果\\GH-GAT\\scr\\temperature_dependency\\continuous_extrapolation'
...
for i in range(df_test.shape[0]):
    keys_test_T.append(
        df_test['Solvent_name'].iloc[i] + '_' +
        df_test['Solute_name'].iloc[i] + '_' +
        str(df_test['T'].iloc[i])
    )
...
for mode, title in zip(modes, titles):
    csv_path = os.path.join(folder, mode + '_predictions.csv')
    ...
    plt.xlabel('Experimental $ln(\gamma_{ij}^{\infty})$', fontsize=18)
    plt.ylabel('Predicted $ln(\gamma_{ij}^{\infty})$', fontsize=18)
    ax = plt.gca()
    ax.tick_params(axis='both', which='major', labelsize=15)
    cax = fig.add_axes([0.15, 0.81, 0.5, 0.05])
    fig.colorbar(im, cax=cax, orientation='horizontal')
    plt.close(fig)
    fig.savefig(folder + '/parity_' + mode + '_' + method + '.png',
                dpi=300, format='png')
    fig.savefig(folder + '/parity_' + mode + '_' + method + '.svg',
                dpi=300, format='svg')


# Parity plot for interpolation and extrapolation in subplots
method = 'GHGEAT'
folder = 'D:\\化工预测\\论文复现结果\\GH-GAT\\scr\\temperature_dependency\\continuous_extrapolation'
modes = ['interpolation', 'lower_extrapolation', 'upper_extrapolation']
titles = ['Interpolation', 'Lower extrapolation', 'Upper extrapolation']
...
for mode in modes:
    csv_path = os.path.join(folder, mode + '_predictions.csv')
    df = pd.read_csv(csv_path)
    distances_lst.extend(df['Distance'].tolist())
vmin = min(distances_lst)
vmax = max(distances_lst)


for mode, title, ax in zip(modes, titles, axs):
    csv_path = os.path.join(folder, mode + '_predictions.csv')
    df = pd.read_csv(csv_path)
    df = df.sort_values('Distance')
    y_true = df['log-gamma'].to_numpy()
    ...
    ax.plot([np.min(y_true), np.max(y_true)],
              [np.min(y_true), np.max(y_true)], '--', c='0.5')
    ax.grid(True, which='major', color='gray', linestyle='dashed', alpha=0.3)
    im = ax.scatter(y_true, y_pred, 
                s=40, 
                edgecolors='k', 
                alpha=0.5, 
                marker='o', 
    ...
    ax.set_xlabel('Experimental $ln(\gamma_{ij}^{\infty})$', fontsize=18)
    if jj == 0:
        ax.set_ylabel('Predicted $ln(\gamma_{ij}^{\infty})$', fontsize=18)
    ax.tick_params(axis='both', which='major', labelsize=15)
    jj += 1
...
plt.close(fig)
fig.savefig(folder + '/parity_complete_' + method + '.png',
            dpi=300, format='png')
fig.savefig(folder + '/parity_complete_' + method + '.svg',
            dpi=300, format='svg')