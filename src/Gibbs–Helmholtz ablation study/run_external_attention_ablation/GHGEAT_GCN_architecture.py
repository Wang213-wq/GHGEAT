"""
GHGEAT_GCN架构 - 使用GCN替换External Attention的消融实验版本
"""
import numpy as np
import torch
...
class GCNLayer(nn.Module):
    """
    使用GCN (Graph Convolutional Network) 替换External Attention
    """
...
class NodeModel(torch.nn.Module):
    def __init__(self, v_in, u_in, hidden_dim, memory_size=128, attention_weight=1.0):
        super().__init__()
        # 使用GCN替换External Attention
        self.gcn_layer = GCNLayer(v_in + u_in, hidden_dim)
    ...
    def forward(self, x, edge_index, edge_attr, u, batch):
        ...
        # 使用GCN处理节点特征
        gcn_out = self.gcn_layer(combined_input, edge_index, batch)
        ...
        if self.attention_weight >= 1.0:
            ...
            if self.input_projection is None or self._input_proj_dim != combined_dim:
                ...
                self.input_projection = nn.Linear(combined_dim, gcn_out.size(1)).to(combined_input.device)
            ...
            if self.attention_weight <= 0.0:
                ...
                out = self.attention_weight * gcn_out + (1.0 - self.attention_weight) * original_proj
...
class GHGEAT_GCN(nn.Module):
    """
    GHGEAT_GCN: 使用GCN替换External Attention的消融实验版本
    """
    def __init__(self, v_in, e_in, u_in, hidden_dim, attention_weight=1.0):
        super(GHGEAT_GCN, self).__init__()
        self.attention_weight = attention_weight
