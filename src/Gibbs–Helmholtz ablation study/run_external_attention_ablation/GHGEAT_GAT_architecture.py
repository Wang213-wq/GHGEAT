"""
GHGEAT_GAT架构 - 使用GAT替换External Attention的消融实验版本
"""
import numpy as np
import torch
...
class GATLayer(nn.Module):
    """
    使用GAT (Graph Attention Network) 替换External Attention
    """
    def __init__(self, in_features, hidden_dim, num_heads=8, dropout=0.1):
        ...
        # 如果hidden_dim * num_heads != hidden_dim，需要投影层
        if hidden_dim * num_heads != hidden_dim:
    ...
    def forward(self, x, edge_index, batch):
        ...
        batch: (num_nodes,)
        """
        # GAT需要edge_index，但我们需要为每个图创建内部边
        # 由于我们在NodeModel中调用，需要从batch信息构建边
...
class NodeModel(torch.nn.Module):
    def __init__(self, v_in, u_in, hidden_dim, memory_size=128, attention_weight=1.0):
        super().__init__()
        # 使用GAT替换External Attention
        self.gat_layer = GATLayer(v_in + u_in, hidden_dim, num_heads=8, dropout=0.1)
    ...
    def forward(self, x, edge_index, edge_attr, u, batch):
        ...
        # 使用GAT处理节点特征
        # GAT需要edge_index，我们使用传入的edge_index
        gat_out = self.gat_layer(combined_input, edge_index, batch)
        
        # 根据attention_weight调整输出
        if self.attention_weight >= 1.0:
            ...
            if self.input_projection is None or self._input_proj_dim != combined_dim:
                ...
                self.input_projection = nn.Linear(combined_dim, gat_out.size(1)).to(combined_input.device)
            ...
            if self.attention_weight <= 0.0:
                ...
                out = self.attention_weight * gat_out + (1.0 - self.attention_weight) * original_proj
...
class GHGEAT_GAT(nn.Module):
    """
    GHGEAT_GAT: 使用GAT替换External Attention的消融实验版本
    """
    def __init__(self, v_in, e_in, u_in, hidden_dim, attention_weight=1.0):
        super(GHGEAT_GAT, self).__init__()
        self.attention_weight = attention_weight
