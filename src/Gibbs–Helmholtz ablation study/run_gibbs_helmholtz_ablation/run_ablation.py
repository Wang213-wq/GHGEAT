"""
Gibbs-Helmholtz消融实验运行脚本
运行GHGEAT_Baseline和GHGEAT_NoGH两个实验
"""
import sys
import os
...
# 导入消融实验架构
sys.path.insert(0, str(Path(__file__).parent / "GHGEAT_NoGH"))
from GHGEAT_NoGH_architecture import GHGEAT_NoGH
...
def run_gibbs_helmholtz_ablation():
    """
    运行Gibbs-Helmholtz消融实验
    包括：
    1. GHGEAT_Baseline: 完整模型 (A + B/T)
    2. GHGEAT_NoGH: 移除Gibbs-Helmholtz项 (只输出A)
    """
    # 数据路径
    ...
    save_base_dir = PROJECT_ROOT / "ablation_results" / "rigorous_ablation"
    ...
    experiments = [
        {
            ...
            'model_class': GHGEAT,
            'description': '完整GHGEAT模型 (A + B/T)'
        },
        {
            'name': 'GHGEAT_NoGH',
            'model_class': GHGEAT_NoGH,
            'description': '移除Gibbs-Helmholtz项 (只输出A)'
        }
    ]
    
    # 运行实验
    print("="*60)
    print("开始运行Gibbs-Helmholtz消融实验")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("所有Gibbs-Helmholtz消融实验完成")
    print("="*60)


if __name__ == '__main__':
    run_gibbs_helmholtz_ablation()
