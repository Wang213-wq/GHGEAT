...
#     abs_path = 'F:\\化工预测\\论文复现结果\\GH-GNN\\models\\isothermal'
#     path = os.path.join(abs_path, model_name)
...
#
#     # Open report file
#     report = open('F:\\化工预测\\论文复现结果\\GH-GNN\\reports\\Report_training_kfolds_' + model_name + '.txt','w')
...
def train_SolvGNN(df, model_name, hyperparameters):
    abs_path = 'F:\\化工预测\\论文复现结果\\GH-GAT\\scr\\isothermal'
    path = os.path.join(abs_path, model_name)
    ...
    # Open report file
    report = open('F:\\化工预测\\论文复现结果\\GH-GAT\\reports\\Report_training_' + model_name + '.txt', 'w')
    def print_report(string, file=report):
        print(string)
        file.write('\n' + string)
...
for T in Ts:
    ...
    # just representative
    csv_path = os.path.join('F:\\化工预测\\论文复现结果\\GH-GAT\\scr\\isothermal'+'\\'+str(T)+'\\'+str(T)+'_train.csv')
