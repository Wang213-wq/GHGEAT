...
class MPNNconv(nn.Module):
    def __init__(self, node_in_feats, edge_in_feats, node_out_feats,
        ...
        edge_network = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(edge_hidden_feats, node_out_feats*node_out_feats) #假设node_out_feats为8,则这里的输出特征为64维
        )
...
class Task_B(nn.Module):
    ...
    def forward(self, xg):
        B = self.activation(self.mlp1b(xg))

        B = self.activation(self.mlp2b(B))

        B = self.mlp3b(B)

        return B

class GHGNN_MTL(nn.Module):
    def __init__(self,v_in, e_in, u_in, hidden_dim):
        super(GHGNN_MTL,self).__init__()
        self.shared_layer = Shared_Layer(v_in, e_in, u_in, hidden_dim)
    ...
    def __getitem__(self, index):
        ...
        # 如果索引不在可以访问的范围内，报告索引错误
            ...
            raise IndexError("Index is out of range for the task of GHGNN model")
    def forward(self,solvent, solute):
