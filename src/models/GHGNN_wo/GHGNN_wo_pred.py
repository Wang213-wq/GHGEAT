...
from GHGNN_wo_architecture import GHGNN_wo
import torch
import os
import numpy as np
...
def pred_GNNGH_T(df, model_name, hyperparameters):
    ...
    model.load_state_dict(torch.load('F:\\化工预测\\论文源代码\\GH-GNN\\models\\temperature_dependency\\GHGNN_wo.pth',
...
for e in epochs:
    ...
    # df = pd.read_csv('F:\\化工预测\\论文复现结果\\GH-GNN\\data\\processed\\molecule_train.csv')
    ...
    # Models trained on the complete train/validation set
    # print('Predicting with ' + model_name)
    # df = pd.read_csv('F:\\化工预测\\论文复现结果\\GH-GNN\\data\\processed\\brouwer_edge_test.csv')
    # df_pred = pred_GNNGH_T(df, model_name=model_name,
    #                   hyperparameters=hyperparameters_dict)
